...
void setup() {
  ...
  file = new SoundFile(this, "scarySFX.wav");
  file.amp(1);
  
  laughing = loadImage("laughing.jpg");
  image1 = loadImage("scaryImage1.jpg");
  image2 = loadImage("scaryImage2.jpg");
  image3 = loadImage("scaryImage3.jpg");
  image4 = loadImage("scaryImage4.jpg");
  image5 = loadImage("scaryImage5.jpg");
}
...
void draw() {
      ...
      for (int i = 0; i < 4; i++) {
        if (b.findRow() != -1) {
          ...
          if (lineClearAnimationToggle) {
            float rand = random(5);
            if (rand < 1) {
              lineClearImage = image1;
            } else if (rand < 2) {
              lineClearImage = image2;
            } else if (rand < 3) {
              lineClearImage = image3;
            } else if (rand < 4) {
              lineClearImage = image4;
            } else if (rand < 5) {
              lineClearImage = image5;
            }
            file.play();
  ...
  // if (speedTemp >= speed && landBuffer == 0){
    p.decrementPos();
    speedTemp = 0;
  }
  speedTemp++;
